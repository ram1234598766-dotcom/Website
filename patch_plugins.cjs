...
const targetPlugins = `const DEFAULT_PLUGINS: Record<string, PluginMeta> = {
...
const replacementPlugins = `const DEFAULT_PLUGINS: Record<string, PluginMeta> = {
  prettier: { name: 'Prettier', description: 'Auto-formatter for JS, HTML, CSS', active: true },
  eslint: { name: 'ESLint', description: 'JavaScript Linter', active: false }
};`;

code = code.replace(targetPlugins, replacementPlugins);

// Remove debugger button and state
code = code.replace(/const \[debuggerActive, setDebuggerActive\] = useState\(false\);\n/g, '');
code = code.replace(/<button\s*onClick=\{\(\) => setDebuggerActive\(!debuggerActive\)\}[\s\S]*?Debugger \{debuggerActive \? 'On' : 'Off'\}\s*<\/button>/g, '');
code = code.replace(/\{debuggerActive && \([\s\S]*?\}\)/, '');
