...
with Firebase Realtime Database and GitHub integration. The site is a Next.js
15 App Router app (`output: 'standalone'`) bundled for a single Cloudflare
Worker via OpenNext — there is no static `out/` export. This guide covers how
to set up, run, and ship changes — and the ground rules we review against.
...
4. Run `npm run build` if the change touches pages, data plumbing, or build
   config (the production build must still succeed).
...
| `npx playwright test --config=tests/e2e/playwright.config.ts` | E2E suite — the webServer builds and serves the app (`next build && next start -p 4173`) |
| `npm run build`      | Production build: `next build` + OpenNext Worker bundle (`.open-next/`) |
...
- Vitest: 1403/1403 tests across 93 files (Sep 2026), including
  `tests/phase1`, `tests/phase2`, `tests/phase5` (the no-hardcoded-data
  contract), `tests/phase-schema`, plus telemetry/Sentry/LogRocket audits
  and plugin/manifest/edge/operations coverage.
...
| Path                    | Purpose                                                        |
|-------------------------|----------------------------------------------------------------|
| `app/`                  | Next.js pages and layout (App Router)                          |
...
| `workers/`              | `worker.ts` (the Worker that serves the app shell, static assets, and API routes) and `grants.ts` |
| `tests/` + `tests/e2e/flows/` | Unit/component tests and Playwright E2E flows      |
| `docs/`                 | Architecture, roadmap, and audit documentation                |

## Common pitfalls

- **Deploy rules from `database.rules.json`.** `npm run firebase:deploy` runs
  `firebase deploy --only database` — the live tier is Realtime Database;
  there is no Firestore in this project.
- **E2E builds and serves the app.** The Playwright webServer runs
  `next build && next start -p 4173`, so no separate build step is needed
  before `npx playwright test --config=tests/e2e/playwright.config.ts`.
